...
'''Class to implement windowed (or not!) sinc interpolation. Options include Lanczos, Hann, and Kaiser. Possibly more. Copyright Ian Czekala 2013.

Assumptions: sinc interpolation is useful for *exactly* recreating a band-limited function which has been discretely sampled at *regular* intervals. However, true sinc interpolation requires an infinite series of points, and therefore we must window the function. Also, we can relax the requirement that the samples need to be equally spaced at all points and assume that as long as the samples can be treated as equally spaced within the windowed kernel, then we should be fine.
