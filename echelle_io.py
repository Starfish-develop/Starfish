...
def rechellenp(bname):
    '''Reads text files similar to rechelletxt, but instead returns a 3D numpy array that takes on the shape (
    norders, 2, len_wl). For example, GWOri has shape (51,2,2304). Assumes each order is the same length.'''
    inp = bname + "/01.txt"
    wl, fl = np.loadtxt(inp, unpack=True)
    len_wl = len(wl)
...
def rechellenpflat(bname):
    '''Reads text files similar to rechelletxt, but instead returns two 2D numpy arrays of shape (norders,
    len_wl). The first is wl, the second fl. For example, GWOri has shape (51,2304). Assumes each order is the same
    length.'''
    inp = bname + "/01.txt"
    wl, fl = np.loadtxt(inp, unpack=True)
    len_wl = len(wl)
...
def make_bool_masks(fname):
    '''Takes the masks from load_masks() and turns these into boolean arrays that can be easily applied when doing
    chi^2 comparisons or plots.'''
