...
parser.add_argument(
    "--use_cov",
    action="store_true",
    help="Use the local optimal jump matrix if present.",
)
...
DataSpectra = [
    Spectrum.open(os.path.expandvars(file), orders=config.data["orders"])
...
class Order:
    ...
    def initialize(self, key):
        ...
        self.EIGENSPECTRA = np.vstack(
            (
                self.pca.flux_mean[np.newaxis, :],
                self.pca.flux_std[np.newaxis, :],
                self.pca.eigenspectra,
            )
        )
    ...
    def evaluate(self):
        ...
        X = (self.chebyshevSpectrum.k * self.flux_std * np.eye(self.ndata)).dot(
            self.eigenspectra.T
        )
    ...
    def CC_debugger(self, CC):
        ...
        if CC.diagonal().min() < 0.0:
            ...
            print(
                *[
                    "{: >6} {:{fill}>20.3e}".format(
                        i, evals_CC[i], fill=mark(evals_CC[i])
                    )
                    for i in range(10)
                ],
                sep="\n"
            ...
            print(
                *[
                    "{: >6} {:{fill}>20.3e}".format(
                        n_tot - 10 + i, evals_CC[-10 + i], fill=mark(evals_CC[-10 + i])
                    )
                    for i in range(10)
                ],
                sep="\n"
    ...
    def update_Theta(self, p):
        ...
        for lres, hres in zip(
            chain([self.flux_mean, self.flux_std], self.eigenspectra), eigenspectra_full
        ):
