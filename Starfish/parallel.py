...
parser.add_argument("--incremental_save", type=int, default=0,
                    help="How often to save incremental progress of MCMC samples.")
...
DataSpectra = [DataSpectrum.open(os.path.expandvars(file), orders=config.data["orders"]) for file in
               config.data["files"]]
...
logging.basicConfig(format="%(asctime)s - %(levelname)s - %(name)s -  %(message)s", filename="{}log.log".format(
    config.outdir), level=logging.DEBUG, filemode="w", datefmt='%m/%d/%Y %I:%M:%S %p')
#
...
class Order:
    ...
    def initialize(self, key):
        ...
        self.EIGENSPECTRA = np.vstack(
            (self.pca.flux_mean[np.newaxis, :], self.pca.flux_std[np.newaxis, :], self.pca.eigenspectra))
        ...
        self.noutdir = config.outdir + "{}/{}/".format(self.spectrum_id, self.order)
...
class SampleThetaCheb(Order):
    ...
    def finish(self, *args):
        super().finish(*args)
        fname = config.outdir + config.specfmt.format(self.spectrum_id, self.order) + "/mc.hdf5"
