...
if args.create:
    ...
    if (config.data["grid_name"] == "PHOENIX") & (len(config.grid['parname']) == 3):
        mygrid = eval("Starfish.grid_tools." + config.data["grid_name"]+ "GridInterfaceNoAlpha")(air=air)
    else:
        mygrid = eval("Starfish.grid_tools." + config.data["grid_name"]+ "GridInterface")(air=air)

    hdf5_path = os.path.expandvars(config.grid["hdf5_path"])
...
if args.plot:
    ...
    def plot(par_flux):
        ...
        fmt = "=".join(["{:.2f}" for i in range(len(config.grid['parname']))])
        name = fmt.format(*[p for p in par])
        fig.savefig(config["plotdir"] + "g" + name + ".png")
