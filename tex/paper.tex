...
$T_{\rm eff}$ & 6517 & 13 K \\
$\log_{10} g$\tablenotemark{a} & \multicolumn{2}{c}{4.29 dex} \\
$\Z$ & -0.27 & 0.01 dex \\
$v \sin i$ & 4.29 & 0.05 $\kms$\\
$v_z$ & -4.62 & 0.02 $\kms$ \\
$\log_{10} \Omega$\tablenotemark{b} & -12.7181 & 0.0004\\
...
$\log_{10} \cc{24}{0}$\tablenotemark{a} & \multicolumn{2}{c}{0.00} \\
$\cc{24}{1}$ & -0.010 & 0.002 \\
$\cc{24}{2}$ & -0.011 & 0.001\\
$\cc{24}{3}$ &  0.000 & 0.001\\
\\
...
$l_{24}$ & 7.3 & 0.6 $\kms$ \\
\enddata
\tablenotetext{a}{parameter value fixed}
\tablenotetext{b}{The {\sc CfA/Kurucz} library is scaled such that the average flux is unity, rather than given in units of flux at the stellar surface, so this value is an unknown multiple of $R^2/d^2$.}
\end{deluxetable}
...
$l_{23}$ & 9.8 & 0.5 $\kms$\\
\cutinhead{Order 24 nuisance parameters}
$\cc{24}{0}$\tablenotemark{a} & \multicolumn{2}{c}{0.00} \\
$\cc{24}{1}$ & -0.009 & 0.003 \\
$\cc{24}{2}$ & -0.012 & 0.003 \\
$\cc{24}{3}$ & -0.001 & 0.002 \\
$b_{24}$ & 1.00 & 0.02 \\
...
for the 2.205\,$\mu$m \ion{Na}{1} doublet and 2.263\,$\mu$m \ion{Ca}{1}
...
\begin{deluxetable}{lr@{$\pm$}l} 
\tablecaption{\label{table:Gl51}Gl51 {\sc Phoenix} results}
\tablehead{\colhead{Parameter} & \multicolumn{2}{c}{Value}}
\startdata
\cutinhead{Stellar parameters}
$T_{\rm eff}$ & 2998  & 17 K \\
$\log_{10} g$\tablenotemark{a} & \multicolumn{2}{c}{5.0 dex} \\
$\Z$ & -0.01 & 0.04 dex \\
$v \sin i$ & 5.68 & 4.15 $\kms$\\
$v_z$ & 12.1 & 1.3 $\kms$ \\
$\log_{10} \Omega$ & -19.628  & 0.003 \\
\cutinhead{Nuisance parameters}
$\log_{10} c_0$\tablenotemark{a} & \multicolumn{2}{c}{0.0} \\
$c_1$ & -0.011 & 0.002 \\
$c_2$ & 0.005 & 0.002 \\
$c_3$ & -0.002 & 0.001 \\
$b$ & 1.56 & 0.14 \\
...
\begin{center}
  ...
  \caption{Gl51 stellar posterior marginalized over the other nuisance parameters, including the local covariance kernels which account for the \ion{Na}{1} and \ion{Ca}{1} lines.}
...
We find, like \citet{rojas-ayala12} do, that the {\sc PHOENIX} models manage to fit the overall spectral shape well, but consistently under-predict the strength of the \ion{Na}{1} and \ion{Ca}{1} lines, even when using high metallicity models. \citet{rajpurohit10} speculate that this discrepancy may result from inaccuracies in the oscillator strengths and opacities for the \ion{Na}{1} and \ion{Ca}{1} lines. Fortunately, the local kernels provide a mechanism for self-consistently determining the weights of discrepant spectral lines, like that speculated about by \citet{mann13}, such that they bring their full information content to bear on the stellar posterior distribution without bias. Figure~\ref{fig:Gl51_residuals} shows that these kernels have identified these two line groups as problematic and appropriately inflated the covariance for this region of spectrum, reducing the ability of these lines to bias the final stellar parameters. 
