...
\shorttitle{Robust Spectroscopic Inference}
...
\end{abstract}
\keywords{stars: fundamental parameters --- techniques: spectroscopic --- stars: late-type --- 
...
\begin{figure}[!t]
\begin{center}
  \includegraphics{kernels.pdf}
  \includegraphics{high2low.pdf}
...
shapes.  By using Chebyshev polynomials ({\it middle, top}) one can correct for that mismatch by
adjusting the data ({\it middle, bottom}); instead, in practice we equivalently distort the model
by these polynomials ({\it bottom}) such that the model remains linear in the Chebyshev coefficients
(Eq.~\ref{eqn:chebyshev}).  \label{fig:chebyshev}}
\end{figure}
...
offsets of $\lesssim 8$ pixels, demonstrating clearly that the residuals are not well described by 
...
\begin{figure*}[!htb]
\begin{center}
\includegraphics[scale=1.05]{matrix_compilation.pdf}
...
of a global kernel ($\mathcal{K}^\textrm{G}$) more closely approximates the structure and amplitude of 
the residuals, but misses the outlier line at 5202.2\,\AA.  Including a local kernel 
($\mathcal{K}^\textrm{L}$) at that location results in a covariance structure that does an excellent job 
...
Alternatively, one can also think of adding in this Gaussian component to the covariance matrix as
being equivalent to marginalizing over the existence of an outlier spectral line with a prior on the
line strength that is strictly Gaussian, where adjusting the hyperparameters of the local kernel
changes the prior on the strength and shape of the outlier residual. We find that once such a local
kernel is instantiated at the location of an outlier spectral line, the inference on the stellar
parameters ($\vT$) is moderately robust to the precise values of the kernel parameters. This means
that the marginalization over outlier lines can be performed at minimal computational cost, since
MCMC iterations which would otherwise be spent exploring the parameter space of the local kernel
hyperparameters are largely unnecessary if one is only interested in the stellar parameters. For
completeness, however, the local kernel hyperparameters of the spectral fits presented in
Section~\ref{sec:examples} are allowed to float and explored simultaneously with the other stellar
and nuisance parameters.

\begin{figure}[!t]
\begin{center}
\includegraphics{badlines.pdf}
...
\begin{figure}[!b]
  ...
  \figcaption{A probabilistic graphical model representing how the parameters of the model are
    combined to forward model a spectrum.  When interpolating models using a spectral emulator
    (Appendix \ref{sec:Appendix}), the stellar parameters ($\vt_\ast$) determine the weights
    ($\mathbf{w}$) of the eigenspectra ($\mathbf{\Xi}$), which are modified according to the
    observational parameters ($\vt_\textrm{obs}$) and polynomial parameters ($\cheb$). Together,
    these parameters specify the model spectrum ($\vM$). If one uses linear interpolation instead of
    a spectral emulator, then there would be no intermediate nodes for $\mathbf{w}$ and $\mathbf{\Xi}$. The structure
    of the covariance matrix, which is included in the likelihood function, is determined by the
    covariance hyperparameters ($\cov$). Together, the model spectrum and the covariance matrix
    predict the resulting dataset ($\vD$).
\label{fig:PGM}}
\end{figure}
...
empirically calibrated to accurately reproduce the optical spectrum around the \ion{Mg}{1}\,b triplet at 
...
\begin{figure*}[!t]
\includegraphics{residuals_Kurucz_logg.pdf}
\vspace{-0.3cm}
...
\begin{figure*}[!htb]
\includegraphics{residuals_PHOENIX_logg.pdf}
\vspace{-0.3cm}
...
\begin{figure}[!b]
\begin{center}
  \includegraphics[width=0.5\textwidth]{PHOENIX_triangle.pdf}
\vspace{-0.5cm}
...
extraordinarily helpful discussions and key insights: Daniel Foreman-Mackey, Guillermo Torres, David
Latham, John Johnson and the ExoLab, Daniel Eisenstein, Rebekah Dawson, Tom Loredo and the ExoStat
group, Allyson Bieryla, and Maxwell Moe.  IC is supported by the NSF Graduate Fellowship and the
Smithsonian Institution. K.M. is supported at Harvard by NSF grant AST-1211196. Figure~\ref{fig:PGM} was generated with
\texttt{Daft}\footnote{\url{http://daft-pgm.org/}} and Figures \ref{fig:Kurucz_posterior},
\ref{fig:PHOENIX_posterior}, and \ref{fig:Gl51_posterior} were generated with \texttt{triangle.py}
\citep{foreman-mackey14}.  This research made extensive use of Astropy \citep{astropy13} and the
Julia programming language \citep{julia12}.
...
\begin{figure*}[!htb]
  \begin{center}
  \includegraphics{pca_reconstruct.pdf}
  \vspace{-0.6cm}
...
spectrum from the \textsc{Phoenix} library ($\vt_\ast = [T_\textrm{eff} = 3000\,\textrm{K}, \log g =
  5.0\,\textrm{dex}, \Z = 0.0\, \textrm{dex}]$) compared with a spectrum reconstructed from a linear 
...
\begin{figure*}[!t]
\begin{center}
  \includegraphics{GP_left.pdf}
  \includegraphics{GP_right.pdf}
  \figcaption{The Gaussian process modelling of the principal component weights for the Gl51
    \textsc{Phoenix} spectral library. (\emph{left}) the blue dots mark the weights
    $\mathbf{w}_5^\textrm{grid}$ of the 5th eigenspectrum $\xi_5$ computed at a one-dimensional
    slice of the spectral library for grid points with $\log g = 5.0$, $\Z = 0.0$ and various values
    of $T_\textrm{eff}$. In reality, the weights are a three-dimensional function of $\vt_\ast$. The
...
{Allard}, F., {Homeier}, D., \& {Freytag}, B. 2012,
  \href{http://dx.doi.org/10.1098/rsta.2011.0269}{\JournalTitle{Royal Society
  of London Philosophical Transactions Series A}, 370, 2765}

\bibitem[{{Allard} {et~al.}(2013){Allard}, {Homeier}, {Freytag},
  {Schaffenberger}, \& {Rajpurohit}}]{allard13}
{Allard}, F., {Homeier}, D., {Freytag}, B., {Schaffenberger}, \& {Rajpurohit},
  A.~S. 2013, \JournalTitle{Memorie della Societa Astronomica Italiana
  Supplementi}, 24, 128
...
\bibitem[{{Astropy Collaboration} {et~al.}(2013){Astropy Collaboration},
  ...
  {Pontzen}, {Ptak}, {Refsdal}, {Servillat}, \& {Streicher}}]{astropy13}
{Astropy Collaboration}, {Robitaille}, T.~P., {Tollerud}, E.~J., {et~al.} 2013,
  \href{http://dx.doi.org/10.1051/0004-6361/201322068}{\JournalTitle{\aap},
  558, A33}

\bibitem[{{Bezanson} {et~al.}(2012){Bezanson}, {Karpinski}, {Shah}, \&
  {Edelman}}]{julia12}
...
\bibitem[{{Buchhave} {et~al.}(2010){Buchhave}, {Bakos}, {Hartman}, {Torres},
  {Kov\'{a}cs}, {Latham}, {Noyes}, {Esquerdo}, {Everett}, {Howard}, {Marcy},
  {Fischer}, {Johnson}, {Andersen}, {F{\H u}r\'{e}sz}, {Perumpilly},
  {Sasselov}, {Stefanik}, {B\'{e}ky}, {L\'{a}z\'{a}r}, {Papp}, \&
  {S\'{a}ri}}]{buchhave10}
{Buchhave}, L.~A., {Bakos}, G.~A., {Hartman}, J.~D., {et~al.} 2010,
  \href{http://dx.doi.org/10.1088/0004-637X/720/2/1118}{\JournalTitle{\apj},
  720, 1118}
...
\bibitem[{{Buchhave} {et~al.}(2012){Buchhave}, {Latham}, {Johansen},
  ...
  {Robertson}, {Shporer}, {Stefanik}, {Still}, \& {Quinn}}]{buchhave12}
{Buchhave}, L.~A., {Latham}, D.~W., {Johansen}, A., {et~al.} 2012,
  \href{http://dx.doi.org/10.1038/nature11121}{\JournalTitle{\nat}, 486, 375}
...
{Castelli}, F., \& {Kurucz}, R.~L. 2004, \JournalTitle{ArXiv Astrophysics
  e-prints}, \href{http://arxiv.org/abs/astro-ph/0405087}{{\sffamily
  astro-ph/0405087}}

\bibitem[{{Cushing} {et~al.}(2005){Cushing}, {Rayner}, \& {Vacca}}]{cushing05}
{Cushing}, M.~C., {Rayner}, J.~T., \& {Vacca}, W.~D. 2005,
  \href{http://dx.doi.org/10.1086/428040}{\JournalTitle{\apj}, 623, 1115}

\bibitem[{{Cushing} {et~al.}(2004){Cushing}, {Vacca}, \& {Rayner}}]{cushing04}
...
\bibitem[{{F{\H{u}}r\'{e}sz}(2008)}]{furesz08}
{F{\H{u}}r\'{e}sz}, G. 2008
...
\bibitem[{{Habib} {et~al.}(2007){Habib}, {Heitmann}, {Higdon}, {Nakhleh}, \&
  {Williams}}]{habib07}
...
\bibitem[{{Heitmann} {et~al.}(2009){Heitmann}, {Higdon}, {White}, {Habib},
  {Williams}, {Lawrence}, \& {Wagner}}]{heitmann09}
...
\bibitem[{{Howard} {et~al.}(2010){Howard}, {Marcy}, {Johnson}, {Fischer},
  {Wright}, {Isaacson}, {Valenti}, {Anderson}, {Lin}, \& {Ida}}]{howard10}
{Howard}, A.~W., {Marcy}, G.~W., {Johnson}, J.~A., {et~al.} 2010,
  \href{http://dx.doi.org/10.1126/science.1194854}{\JournalTitle{Science}, 330,
  653}

\bibitem[{{Husser} {et~al.}(2013){Husser}, {Wende-von Berg}, {Dreizler},
  {Homeier}, {Reiners}, {Barman}, \& {Hauschildt}}]{husser13}
{Husser}, T.-O., {Wende-von Berg}, S., {Dreizler}, S., {et~al.} 2013,
  \href{http://dx.doi.org/10.1051/0004-6361/201219058}{\JournalTitle{\aap},
  553, A6}

\bibitem[{{Ivezi\'{c}} {et~al.}(2013){Ivezi\'{c}}, {Connolly}, {VanderPlas}, \&
  {Gray}}]{ivezic13}
{Ivezi\'{c}}, {\.Z}., {Connolly}, A., {VanderPlas}, J., \& {Gray}, A. 2013,
  {Statistics, Data Mining, and Machine Learning in Astronomy}

\bibitem[{{Johnson} {et~al.}(2007){Johnson}, {Butler}, {Marcy}, {Fischer},
  {Vogt}, {Wright}, \& {Peek}}]{johnson07}
{Johnson}, J.~A., {Butler}, R.~P., {Marcy}, G.~W., {et~al.} 2007,
  \href{http://dx.doi.org/10.1086/521720}{\JournalTitle{\apj}, 670, 833}

\bibitem[{{Joshi} {et~al.}(2009){Joshi}, {Pollacco}, {Collier Cameron},
  {Skillen}, {Simpson}, {Steele}, {Street}, {Stempels}, {Christian}, {Hebb},
  {Bouchy}, {Gibson}, {H\'{e}brard}, {Keenan}, {Loeillet}, {Meaburn}, {Moutou},
  {Smalley}, {Todd}, {West}, {Anderson}, {Bentley}, {Enoch}, {Haswell},
  {Hellier}, {Horne}, {Irwin}, {Lister}, {McDonald}, {Maxted}, {Mayor},
  {Norton}, {Parley}, {Perrier}, {Pont}, {Queloz}, {Ryans}, {Smith}, {Udry},
  {Wheatley}, \& {Wilson}}]{joshi09}
{Joshi}, Y.~C., {Pollacco}, D., {Collier Cameron}, A., {et~al.} 2009,
  \href{http://dx.doi.org/10.1111/j.1365-2966.2008.14178.x}{\JournalTitle{\mnras},
  392, 1532}

\bibitem[{{Koleva} {et~al.}(2009){Koleva}, {Prugniel}, {Bouchard}, \&
  {Wu}}]{koleva09}
{Koleva}, M., {Prugniel}, P., {Bouchard}, A., \& {Wu}, Y. 2009,
  \href{http://dx.doi.org/10.1051/0004-6361/200811467}{\JournalTitle{\aap},
  501, 1269}

\bibitem[{{Kurucz}(1993)}]{kurucz93}
{Kurucz}, R.~L. 1993, {SYNTHE spectrum synthesis programs and line data}

\bibitem[{{Mann} {et~al.}(2013){Mann}, {Gaidos}, \& {Ansdell}}]{mann13}
{Mann}, A.~W., {Gaidos}, E., \& {Ansdell}, M. 2013,
  \href{http://dx.doi.org/10.1088/0004-637X/779/2/188}{\JournalTitle{\apj},
  779, 188}

\bibitem[{{Nidever} {et~al.}(2012){Nidever}, {Zasowski}, {Majewski}, {Bird},
  {Robin}, {Martinez-Valpuesta}, {Beaton}, {Sch\"{o}nrich}, {Schultheis},
  {Wilson}, {Skrutskie}, {O'Connell}, {Shetrone}, {Schiavon}, {Johnson},
  {Weiner}, {Gerhard}, {Schneider}, {Allende Prieto}, {Sellgren}, {Bizyaev},
  {Brewington}, {Brinkmann}, {Eisenstein}, {Frinchaboy}, {Elia Garc\'{\i}a
  P\'{e}rez}, {Holtzman}, {Hearty}, {Malanushenko}, {Malanushenko}, {Muna},
  {Oravetz}, {Pan}, {Simmons}, {Snedden}, \& {Weaver}}]{nidever12}
{Nidever}, D.~L., {Zasowski}, G., {Majewski}, S.~R., {et~al.} 2012,
  \href{http://dx.doi.org/10.1088/2041-8205/755/2/L25}{\JournalTitle{\apjl},
  755, L25}

\bibitem[{{Paxton} {et~al.}(2011){Paxton}, {Bildsten}, {Dotter}, {Herwig},
  {Lesaffre}, \& {Timmes}}]{paxton11}
{Paxton}, B., {Bildsten}, L., {Dotter}, A., {et~al.} 2011,
  \href{http://dx.doi.org/10.1088/0067-0049/192/1/3}{\JournalTitle{\apjs}, 192,
  3}

\bibitem[{{Rajpurohit} {et~al.}(2014){Rajpurohit}, {Reyl\'{e}}, {Allard},
  {Scholz}, {Homeier}, {Schultheis}, \& {Bayo}}]{rajpurohit14}
{Rajpurohit}, A.~S., {Reyl\'{e}}, C., {Allard}, F., {et~al.} 2014,
  \href{http://dx.doi.org/10.1051/0004-6361/201322881}{\JournalTitle{\aap},
  564, A90}

\bibitem[{{Rajpurohit} {et~al.}(2010){Rajpurohit}, {Reyl\'{e}}, {Schultheis},
  \& {Allard}}]{rajpurohit10}
...
\bibitem[{Rasmussen \& Williams(2005)}]{rasmussen05}
...
\bibitem[{{Rayner} {et~al.}(2009){Rayner}, {Cushing}, \& {Vacca}}]{rayner09}
...
\bibitem[{{Tonry} \& {Davis}(1979)}]{tonry79}
