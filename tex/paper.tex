...
To illustrate the performance of our technique, we use two datasets with varying properties. The first is a high-resolution optical spectrum of an F-star, WASP-14. The second is a moderate resolution, mid-infrared spectrum of Gl 51.

WASP-14 is a metal-poor F star acquired with the \emph{TRES} instrument \citep{mink11} on the 1.5m telescope at FLWO. The data was previously analyzed in the context of transiting exoplanet hosts by \citet{torres12}. Automated reduction pipeline by Buchhave. Has XX S/N per resolution element in the \ion{Mg}{b} order ($\sim 5100$ \AA). 51 echelle orders spanning from XX to XX.

Gl 51 is a M5 star from the IRTF spectral library \citep{cushing05, rayner09} taken with the SPEX instrument \citep{rayner03}. CITE (Rojas-Ayala) use the H20 -K index, defined over what wl range? We do this as well as more general tests.
...
\paragraph{Test 2}: I think we should fit an optical spectrum of an F star (WASP-14) and a near-IR spectrum of an M star (Gl51). The WASP will be fit with both the Kurucz and PHOENIX spectra, while the M star will be fit with just the PHOENIX. This will show a few things
\begin{itemize}
