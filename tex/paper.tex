...
The procedure outlined above can be a significant computational challenge.  A
typical data spectrum has $N_{\rm pix} > \mathcal{O}(10^3)$, and therefore the
many repeated evaluations of the matrix product $\vR^{\trans} \vC^{-1} \vR$ in
the likelihood calculation can be numerically expensive.  However, because
$\vC$ is a symmetric, positive semi-definite matrix, we can employ Cholesky
factorization to optimize the evaluation of the matrix product ($\vR^{\trans}
\vC^{-1} \vR$) and avoid the direct calculation of the matrix inverse
($\vC^{-1}$) itself.  For echelle spectra with multiple orders or multiple
spectra of the same object, taken with different instruments, the nuisance
parameters for each chunk of spectrum are independent from the other chunks. This
means that the computationally intensive steps of generating a model spectrum
for a specific wavelength range and evaluating the likelihood can be
parallelized. The only segment of the code that needs to be synchronized is the
MCMC proposal of stellar parameters, which are shared between all chunks of the
spectrum. Because all of the computationally expensive steps can be
parallelized, this algorithm can be massively parallelized on a cluster,
enabling simultaneous inference for wide spectral ranges at high resolution or
multiple spectral datasets acquired with different instruments. To sample this
model, we extend the Metropolis-Hastings sampler included in the {\tt emcee}
package \citep{foreman-mackey13} to function within a parallelized blocked
Gibbs sampler.\footnote{\url{https://github.com/iancze/emcee}}\\
...
atmospheres.}
\tablenotetext{b}{For the spectral emulator (Appendix~\ref{sec:Appendix}) the spectra are scaled such that the average flux is unity, rather than given in units of flux at the stellar surface, so this value is an unknown multiple of $R^2/d^2$.}
...
Aside from the structure of the covariance matrix, there is also a moderate difference between the 
...
(by $\sim$75\,K) and lower metallicity than inferred from the {\sc 
...
like those being collected in programs like the APOGEE and HERMES surveys \citep{nidever12, zucker12}), since they provide enormous leverage for identifying and improving the underlying model systematics. \\
...
Interpolation from a library of synthetic spectra is a conceptually straightforward process, but in the 
application discussed here there are some subtleties that require detailed consideration.  
Spectral line formation is a non-linear process, where line shapes are determined by complex 
radiative transfer through a stellar atmosphere.  Any interpolation process aims to bypass this non-linear 
process and predict the behavior of spectral lines at intermediate points based on pre-computed 
...
There are two important factors to consider when interpolating libraries of model stellar 
...
Model library spectra are stored as (1-dimensional) arrays of flux values,
sampled on high resolution wavelength grids (typically $R \gtrsim 100,000$).
In the case of interest here, the collections of model parameters
$\{\vt_{\ast}\}^\textrm{grid} = [\{T_{\rm eff}, \log g,  \Z \}]$ define the
dimensions of the library grid; the full library
$f_{\lambda}(\{\vt_{\ast}\}^\textrm{grid})$ is therefore encapsulated in a
4-dimensional array.  Both the libraries used here have grid spacings of 0.5
dex in $\log g$ and 0.5 dex in $\Z$; the {\sc CfA/Kurucz} library steps by
250\,K in $T_{\rm eff}$, but the {\sc Phoenix} library has finer coverage in
100\,K increments.  

\begin{figure*}[!t]
  \begin{center}
  \includegraphics{figs/pca_reconstruct.pdf}
  \figcaption{ (\emph{top}) The mean spectrum, standard deviation spectrum, and five eigenspectra that form the basis of the \textsc{Phoenix} synthetic library used to model Gl51, generated using a subset of the \textsc{Phoenix} grid parameters space most relevant for M dwarfs. (\emph{bottom}) The original synthetic spectrum from the \textsc{Phoenix} library compared with a spectrum reconstructed from a linear combination of the eigenspectra using Eqn~\ref{eqn:reconstruct} with the weights listed in the figure.
\label{fig:pca_reconstruct}}
  \end{center}
\end{figure*}

The first step in designing an emulator is to decompose the spectral library into an appropriate basis for emulation \citep{habib07, heitmann09}. We chose the principal component basis (PCA) to decompose the synthetic library into a set of ``eigenspectra,'' following the techniques of \citet{ivezic13}. Prior to the PCA decomposition, we first isolate a subset of the library (containing $M$ spectra) with parameter values that will be most relevant to the star at hand. We then standardize these spectra by first subtracting off the mean spectrum and then ``whiten'' the spectra by dividing by the standard deviation measured in each pixel across the grid. The mean spectrum is 
\begin{equation}
...
\end{equation}
where $\{\vt_\ast \}^\textrm{grid}$ denotes the full collection of the $M$ pairings of stellar parameters sampled in the spectral library and $\{\vt_\ast \}^\textrm{grid}_i$ denotes a single set of stellar parameters drawn from this collection. All spectra in the library are then standardized by subtracting the mean spectrum and dividing by the standard deviation spectrum
\begin{equation}
  \hat{f}_\lambda(\{\vt_\ast \}^\textrm{grid}) = \frac{f_\lambda(\{\vt_\ast \}^\textrm{grid}) - \xi_\mu}{\xi_\sigma}
\end{equation}
Then, the eigenspectra are computed from the standardized grid using PCA analysis (\cite{ivezic13}). We decide to truncate our basis to the first $m$ eigenspectra, where $m$ is decided by the minimum number of eigenspectra required to reproduce any spectrum in the grid to better than 2\% accuracy for all pixels; the mean pixel error is generally much smaller than this ($\lesssim 0.5\%$). We denote a single length $N_{\rm pix}$ eigenspectrum as $\xi_k$, with principal component index $k = \{1, 2, \ldots, m\}$. For example, the eigenspectra basis computed for Gl51 using the \textsc{Phoenix} library is shown in Fig~\ref{fig:pca_reconstruct}, top panel.
...
Using the principal component basis, we can lossily reconstruct any spectrum from the library with a linear combination of the eigenspectra
\begin{equation}
  f_\lambda(\{\vt_\ast \}^\textrm{grid}_i) \approx \xi_\mu + \xi_\sigma \sum_{k=1}^m w_k(\{\vt_\ast \}^\textrm{grid}_i) \xi_k
  \label{eqn:reconstruct}
\end{equation}
where $w_k$ is the weight of a given eigenspectrum, $k$. This weight is a three dimensional scalar function which depends on the stellar parameters $\vt_\ast$. We can determine the weight at any grid point in the library by taking the dot product of the standardized synthetic spectrum with the eigenspectrum  
\begin{equation}
  w_k(\{\vt_\ast \}^\textrm{grid}_i) = \sum_\lambda \hat{f}_\lambda(\{\vt_\ast \}^\textrm{grid}_i) \xi_k
\end{equation}
and is generally a smooth function of stellar parameters (see Figure~\ref{fig:GP_interp}, left panel). To simplify notation, we can write the collection of eigenspectra weights in a length-$m$ column vector
\begin{equation}
  \mathbf{w}(\vt_\ast) = 
  \begin{bmatrix}
    w_1(\vt_\ast )\\
...
\begin{equation}
  ...
  \end{bmatrix} .
\end{equation}
Then, we can rewrite Equation~\ref{eqn:reconstruct} as
\begin{equation}
  f_\lambda(\{\vt_\ast \}^\textrm{grid}_i) \approx \xi_\mu + \xi_\sigma I_{N_{\rm pix}} \mathbf{\Xi} \mathbf{w}(\{\vt_\ast \}^\textrm{grid}_i)
  \label{eqn:reconstruct2}
\end{equation}
where $I_{N_{\rm pix}}$ is an ($N_{\rm pix}, N_{\rm pix}$) identity matrix. 

To recapitulate, we have decomposed the spectra in the synthetic library into a principal component basis, allowing us to lossily reconstruct any spectrum in the synthetic library as a weighted linear combination of $m$ eigenspectra. The weight corresponding to each eigenspectrum is a moderately-smooth scalar function of the three stellar parameters. Therefore, to create a spectrum corresponding to a value of stellar parameters which is not in the spectral library, we must interpolate the weights to this new value. In practice, it may be possible to use a traditional interpolation scheme such as spline interpolation to interpolate the weights directly. However, we found that with high-signal to noise spectra (e.g., Gl51 is S/N $> 400$), the error in any given interpolated representation of the spectrum can constitute a significant portion of the uncertainty budget. This, combined with the Nyquist undersampling of the synthetic grid can cause artificial ``peaking'' of the posterior near grid points in the synthetic library, because the interpolated spectrum is not as good as the raw spectrum at the grid point. Even explicitly accounting for interpolation error by doing ``drop-out'' interpolation tests and empricially propagating it forward did not relieve the peaking problem. We address this problem by using a Gaussian process to encapsulate the range of possible interpolated spectra. 

Now, we describe the Gaussian process used for modelling the eigenspectra weights as a function of stellar parameters. In Section~\ref{sec:method}, we used the formalism of Gaussian processes to develop a sophisticated correlated noise model for the data likelihood, which include contributions from a ``global'' covariance kernel to describe the moderate-amplitude correlations present throughout the spectral residuals and several ``local'' covariance kernels to address the highly correlated but-localized spectral residuals originating from pathological errors in the synthetic spectrum.  We now use a separate Gaussian process to serve as an interpolating function for the weights. Because each eigenspectrum weight is modelled independently, there is a new Gaussian process ($\mathcal{GP}$) for each eigenspectrum. For a single eigenspectrum $k$, we denote the collection of $w_k(\{\vt_\ast \}^\textrm{grid}_i)$ evaluated at all the spectra in the synthetic library as $\mathbf{w}_k^\textrm{grid}$. The Gaussian process treats $\mathbf{w}_k$ as a collection of random variables which are drawn from a single joint multi-variate Gaussian distribution \citep{rasmussen05},
\begin{equation}
  \mathbf{w}_k^\textrm{grid} \sim \mathcal{N} \left ( \mathbf{0}, \mathbf{\Sigma}_k^\textrm{grid} \right )
\end{equation}
The covariance matrix of this multi-variate Gaussian distribution is described by a covariance kernel $\mathcal{K}_\textrm{SE}$. Rather than the \matern\ kernel used in Section~\ref{sec:method}, we elect to use a three dimensional squared-exponential covariance kernel
\begin{multline}
  \mathcal{K}_\textrm{SE}(\vt_{\ast, i}, \vt_{\ast, j} | a_\textrm{SE}, l_{T_\textrm{eff}},  l_{\log g}, l_{\Z}) = \\
  a^2 \exp \left ( - \frac{(T_{\textrm{eff}_i} - T_{\textrm{eff}_j})^2}{2 l_{T_\textrm{eff}}^2 }  \right )
  \exp \left (-\frac{(\log g_i - \log g_j)^2}{2 l_{\log g}^2} \right ) \times \\
  \exp \left ( -\frac{ (\Z_i - \Z_j)^2}{2 l_{\Z}^2} \right )
\end{multline}
This kernel is in contrast to the \matern\ kernel used in Section~\ref{sec:method}, which produces a more jagged Gaussian process reminiscent of the spectral residuals. The squared exponential kernel produces a smooth functional form which closely matches the behavior of the eigenspectra weights (Fig~\ref{fig:GP_interp}, left panel). Then the ($M,M$) covariance matrix is 
\begin{equation}
\mathbf{\Sigma}_k^\textrm{grid} = \mathcal{K}_\textrm{SE}( \{\vt_\ast \}^\textrm{grid}, \{\vt_\ast \}^\textrm{grid})
\end{equation}
which represents the evaluation of the covariance kernel for all pairings of grid point stellar parameters, and the conditional dependence on the kernel hyperparameters is implied.

The optimal parameters for the hyperparameters of the squared exponential are given by the multi-variate Gaussian probability distribution, which is called the marginal likelihood function or ``evidence'' for the weights \citep{rasmussen05},
\begin{equation}
...
\end{equation}
Because $\mathbf{\Sigma}_k^\textrm{grid}$ is defined by $\mathcal{K}_\textrm{SE}$ and its hyperparameters, this likelihood function is equivalent to $p(\mathbf{w}_k^\textrm{grid} |\; a_\textrm{SE}, l_{T_\textrm{eff}},  l_{\log g}, l_{\Z})$. By multiplying this with a suitable prior over $a_\textrm{SE}$ and $l$, we arrive at a posterior distribution for the hyperparameters of the Gaussian process. For computational efficiency, we optimize the hyperparameters by finding the peak of the posterior distribution with a downhill gradient algorithm and then consider the hyperparameters fixed.

Now the Gaussian process is specified and we can use it to predict the values of the weights at a new set of stellar parameters $\vt_\ast$ by considering them drawn from joint distribution 
\begin{equation}
  \begin{bmatrix}
  ...
  \mathbf{0},
  \begin{bmatrix}
  \mathbf{\Sigma}_k^\textrm{grid} & \mathcal{K}_\textrm{SE}( \{\vt_\ast \}^\textrm{grid}, \vt_\ast) \\
  \mathcal{K}_\textrm{SE}(\vt_\ast, \{\vt_\ast \}^\textrm{grid}) & \mathcal{K}_\textrm{SE}(\vt_\ast,\vt_\ast)
  \end{bmatrix}
  \right )
\end{equation}
where $\mathcal{K}_\textrm{SE}( \{\vt_\ast \}^\textrm{grid}, \vt_\ast)$ represents a length $M$ column vector where $\mathcal{K}_\textrm{SE}$ is evaluated pairwise with all values in $\{\vt_\ast \}^\textrm{grid}$ and the proposed value of $\vt_\ast$. This vector is stacked horizontally next to  $\mathbf{\Sigma}^\textrm{grid}$, and its transpose is stacked vertically underneath $\mathbf{\Sigma}^\textrm{grid}$. The combination of all of these products results in an augmented covariance matrix of size ($M + 1, M + 1$). The probability distribution of $w_k(\vt_\ast)$ is given by the conditional Gaussian distribution \citep{rasmussen05}
\begin{equation}
...
where
\begin{equation}
 \mu_k =   \mathcal{K}_\textrm{SE}(\vt_\ast, \{\vt_\ast \}^\textrm{grid}) \mathbf{\Sigma}^{\textrm{grid}, -1} \mathbf{w}_k^\textrm{grid}
\end{equation}
\begin{multline}
  \sigma_k = \mathcal{K}_\textrm{SE}(\vt_\ast,\vt_\ast) - \\
  \mathcal{K}_\textrm{SE}(\vt_\ast, \{\vt_\ast \}^\textrm{grid}) \mathbf{\Sigma}^{\textrm{grid}, -1} \mathcal{K}_\textrm{SE}( \{\vt_\ast \}^\textrm{grid}, \vt_\ast)
\end{multline}

Although this equation is complex, the interpretation is straightforward: the probability distribution of $w_k$ is a univariate Gaussian distribution whose mean and variance are a function of $\vt_\ast$, conditional upon the (fixed) values of $\mathbf{w}_k^\textrm{grid}$ and the squared exponential hyperparameters (see Figure~\ref{fig:GP_interp}, right panel).

If we desired an actual value of the interpolated weight, for example to reconstruct a model spectrum, we could simply draw a Gaussian random variable from the probability distribution in Eqn~\ref{eqn:conditional_gp}. However, because we now know the probability distribution of the weight as a function of $\vt_\ast$, we can rewrite our data likelihood function (Eqn~\ref{eqn:lnlikelihood}) in such a way that it is possible to analytically marginalize over all possible values of $w_k$, and thus all probable spectral interpolations. 

Next, we consider all $m$ of the Gaussian processes describing the weight for each eigenspectra collectively. For a given value of $\vt_\ast$, these Gaussian processes deliver a collection of $m$ one-dimensional Gaussian probability distributions. Because we have modelled each $w_k$ independently, the joint probability of the vector of weights $\mathbf{w}$ is simply a multidimensional Gaussian with only the diagonal elements filled
\begin{equation}
  p(\mathbf{w} |\, \vt_\ast) = \mathcal{N} \left ( \mathbf{w} \left | \right . \mathbf{\mu}_\mathbf{w}, \mathbf{\Sigma}_\mathbf{w} \right )
\end{equation}
\begin{equation}
  p(\mathbf{w} |\, \vt_\ast) = \mathcal{N} \left ( \mathbf{w} \left | 
  \begin{bmatrix}
    \mu_1 \\
    \vdots \\
    \mu_m \\
  ...
  \right .
  \right)
  \label{eqn:weight_conditional}
\end{equation}

So far, we have presented the reconstruction of a spectrum as a linear combination of the eigenspectra of the synthetic library (Figure~\ref{fig:pca_reconstruct}). In practice, however, the reconstructed spectrum must be further post-processed to match the observational parameters ($\vt_{\rm obs}$) of the data spectrum by applying convolutions for instrumental broadening, stellar rotation broadening, and Doppler shift, and then lastly re-sampling the spectrum to the exact pixels of the CCD (Section~\ref{subsec:postprocess}). Fortunately, because convolution is a linear operation, we can first post-process the raw eigenspectra according to $\vt_{\rm obs}$, and then represent the reconstructed spectrum as a linear combination of these modified eigenspectra without loss of information. Unfortunately, the Doppler shift and resampling operations are not linear operations, and there will be some loss of information when trying to approximate them in this manner. However, we find that in practice when the synthetic spectra are oversampled relative to the instrument resolution by a reasonable factor, the flux error due to resampling is smaller than 0.2\% across all pixels , and thus any effect is negligible. For notational compactness, we let $\widetilde{\xi}_\mu$, $\widetilde{\xi}_\sigma$, and $\widetilde{\mathbf{\Xi}}$ represent the post-processed eigenspectra, with an implied dependence on the current values of the observational parameters ($\vt_\textrm{obs}$) and the polynomial nuisance parameters $(\phi_\mathsf{P})$. Now, the model spectrum is specified conditional on the vector of eigenspectra weights as 
\begin{equation}
...
\end{equation}
and the likelihood function (Eqn~\ref{eqn:likelihood}) is specified conditional on the weights
\begin{equation}
...
The final task of designing the spectral emulator is to combine this data likelihood function with the posterior distribution of the eigenspectra weights (Eqn~\ref{eqn:weight_conditional}) and then marginalize over the weights
\begin{equation}
  p(\mathsf{D} | \vt_\ast) = \int p(\mathsf{D} | \mathbf{w}) p( \mathbf{w} | \vt_\ast) d \mathbf{w}
  \label{eqn:marginal}
\end{equation}
such that we are left with a modified posterior distribution of the data that incorporates the range of probable interpolation values for the model. To perform this multidimensional integral, we use a convenient lemma found in \citet[Appendix A]{gelman13}: if the probability distributions of $\mathbf{w}$ and $\mathsf{D} | \mathbf{w}$ are specified conditionally as in Eqns~\ref{eqn:weight_conditional} and \ref{eqn:likelihood_conditional}, respectively, then the marginal distribution (Eqn~\ref{eqn:marginal}) is
\begin{equation}
...
\end{equation}
where the dependence on the model parameters is now made explicit.
We can couch this modified likelihood function in the form of Eqn~\ref{eqn:lnlikelihood} by rewriting
\begin{equation}
...
\end{equation}
where $\mathsf{M}^\prime$ can be thought of as the ``mean model spectrum'' given the model parameters, and the covariance matrix has been modified to account for the various probable manifestations of the model spectrum about the mean spectrum.
