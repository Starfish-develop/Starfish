...
\begin{abstract}
We present a modular and extensible framework for inference of model parameters using synthetic spectra. By the nature of continuously sampled spectra, when subtracting a synthetic model, adjacent pixels in the residual spectrum exhibit some degree of correlation. In the high signal-to noise, large spectral range limit common to stellar parameter estimation, the covariant structure of the residuals can bias the resulting parameter determinations. We present a likelihood function designed to account for this covariant structure, utilizing the machinery of Gaussian process covariance kernels to self-consistently identify and model the covariance. Additionally, a common problem specific to stellar parameter estimation is the presence of spectral lines with mismatched strength, often due to incorrect atomic constants or radiative treatment. To address this issue, we also present the development of a novel local covariance kernel that can identify and self-consistently down-weight pathologically wrong spectral line ``outliers.'' By fitting multiple spectra in a hierarchical manner, these local kernels provide a mechanism to learn empirical corrections to spectral models. We demonstrate the application of this method, implemented as a freely available open source code, by fitting an high resolution optical spectrum WASP-14, an F star, and a moderate resolution mid-infrared spectrum of Gl51, an M5 star. 
\end{abstract}
...
The interpolation of a model spectrum from a synthetic library of spectra is a conceptually straightforward process; however, depending on the application, in practice there may be several subtle effects that require consideration. Spectra are generally stored in a synthetic library as 1-D arrays of flux values, sampled at specific wavelengths at high resolution (typically R$\gtrsim$100,000). Each spectrum has several fundamental parameters which describe the model that generated it. For example, the stellar libraries used in this paper have three parameters, $T_\textrm{eff}$, $\log_{10} g$, and $\Z$, each of which constitutes an axis in the synthetic library. Therefore, the full synthetic grid is stored as a 4-D array, where the first three axes are the stellar parameters and the 4th axis describes the wavelength. The value stored in the array is the flux of the spectrum described by the current value of the four axes.

Spectral line formation is a non-linear process: line shapes are determined by the complicated radiative transfer through the stellar atmosphere, the structure of which is iterated to convergence using a specific set of stellar parameters. Interpolating a spectrum short-cuts this non-linear process and attempts to predict the behavior of the spectral line based upon the behavior of nearby spectra in the grid. While this approximation is generally accurate enough for our applications, in the high signal-to-noise regime it becomes necessary to account for any inaccuracies introduced by the interpolation. In practice, there are two factors to consider when interpolating spectra. First, for a small percentage of pixels, the flux value will not vary monotonically with change in fundamental parameters. This informs the decision about which functional form of interpolation to use. Second, inspecting the flux values of a sample of pixels as a function of fundamental parameters makes it apparent that not all pixels of the spectrum are Nyquist sampled with regards to the stellar parameters. This unfortunately means that the synthetic grid is not sampled densely enough to losslessly reconstruct a synthetic spectrum. However, given the computational complexity of stellar synthesis, this fact is not surprising. In order to proceed, we make the most conservative assumption about the behavior of the pixels between grid points and decide to use a linear interpolation scheme. Using a more advanced interpolation method, such as a spline or other band-limited interpolation scheme, would not remedy the fact that the grid itself is not Nyquist sampled. To account for the loss of spectral information during the interpolation process, we build a model of interpolation error and incorporate this into the likelihood function (Eqn.~\ref{eqn:lnlikelihood}).

We empirically determine the interpolation error by performing a series of ``drop-out'' interpolation tests, whereby we remove a spectrum from the grid, generate an interpolated spectrum from the nearest two grid points, and compare the difference between the two. As a concrete example, the {\sc Phoenix} grid used for the WASP-14 parameter estimation is sampled every 100K in $T_\textrm{eff}$ and every 0.5 dex in $\log_{10} g$ and $\Z$. Therefore, for each spectrum in the synthetic grid, there are also three error spectra associated with it, one for each axis of stellar parameters. For example, consider a spectrum with stellar parameters $T_\textrm{eff} = $ 6000K, $\log_{10} g=$4.0 dex, and $\Z = $ -0.5 dex. A temperature ``error spectrum'' would be formed by subtracting this true spectrum from the spectrum formed by the average of the $T_\textrm{eff} = $ 6100K spectrum and the $T_\textrm{eff} = $ 5900K spectrum, with $\log_{10} g$ and $\Z$ held constant. Likewise, the $\log_{10} g$ and $\Z$ error spectra are created by performing the same operation over the $\log_{10} g$ and $\Z$ axes of the grid (see Figure~\ref{fig:interp_errors}). Because the error spectra are determined over an interpolation distance twice that (e.g., 100K) of the actual distance used (e.g., 50K), we reduce the amplitude of the error spectra by two. Drop-out tests at larger interpolation distances (e.g., 200K and 300K) show that the amplitude of the error spectrum grows approximately linearly with interpolation distance. 

\begin{figure}[!t]
\begin{center}
  \includegraphics{figs/interp_errors.pdf}
  \figcaption{(\emph{top}) A small range of the ``true'' {\sc Phoenix} spectrum at $T_\textrm{eff} = $ 6000K, $\log_{10} g=$4.0 dex, and $\Z = $ -0.5 dex, shown with the drop-out interpolation spectra along each of the three parameter axes. These interpolation error spectra are determined over an interpolation distance twice that (e.g., 100K) of the actual distance used while fitting. The interpolated spectra closely match the ``true'' spectrum. (\emph{bottom}) The three ``error spectra'', or difference between the interpolated spectrum and the true spectrum, relative to the true spectrum value. The shaded region shows the total change in flux between the two spectra used for interpolation, relative to the true spectrum. Note that the error spectrum is generally much smaller than the typical change in flux. For the few points where this is not true (e.g., $\log_{10} g$ near 5161.5\AA) this implies a non-monotonic change in flux between grid points. Therefore, it is most conservative to use the error spectrum in an interpolation error model, which accounts for this behavior and protects against assumptions of monotonicity.}
  \label{fig:interp_errors}
\end{center}
\end{figure}

Generally, the interpolation error is small: typically no larger than 2\% of the flux (smaller than the Poisson noise for moderate to high signal-to-noise spectra) and usually--but not always--much smaller than the total flux difference between spectra at adjacent grid points (see Figure~\ref{fig:interp_errors}). This test gives us confidence that it is possible to generate a sufficiently accurate interpolated spectrum, and that inference on the stellar parameters may be drawn at a substantially finer level that the coarsity of the grid spacing itself, especially given the large number of possible lines to fit. However, while the interpolation error is small, because all spectral lines have roughly the same shape, it is often true that the error adds \emph{coherently}. When the signal to noise of the spectrum is particularly high, this coherent trend can contribute substantially by diminishing the likelihood of an interpolated spectrum, since the lines in the spectrum are less ``physical'' in shape than that of a spectrum directly on a gridpoint, which requires minor or no interpolation. Therefore in order to deliver the proper likelihood for interpolated spectra it is necessary to quantify the degree of interpolation error and add this into the covariance matrix. 

\begin{figure}[!t]
\begin{center}
  \includegraphics{figs/interp_grid.pdf}
  \figcaption{Tri-linear interpolation on a grid cell the shape of a rectangular prism. I'm not sure if this figure is necessary, but I think it could help guide the reader. Perhaps by labelling the axis with temperature and so on. It is here as a prototype.}
  \label{fig:interp_grid}
\end{center}
\end{figure}

Because the interpolation error is highly dependent on whether the wavelength is on the continuum or on a spectral line, we must model the covariance of the errors. For example, all pixels at the continuum will tend to act similarly, regardless of where on the spectrum the pixels fall. All pixels that fall on the blue side of a spectral line will tend to exhibit a similar error behaviour regardless of which spectral line they belong to, as long as the general behavior of the spectral lines was similar. The elements of the interpolation covariance matrix $\mathsf{C}_{ij}^\textrm{int}$ are determined empirically from the drop-out spectra, and are a function of the degree of the interpolation. Consider the manner in which a typical spectrum is tri-linearly interpolated from the synthetic grid (Figure~\ref{fig:interp_grid}). The interpolated spectrum is a weighted average of the neighboring 8 spectra, with weights $d_l$ based upon the distance from the nearest grid points $P$. To generate a composite covariance matrix which reflects the interpolation error present for the interpolated spectra, we must also combine the error spectra. Because the error is covariant, we must first determine the empirical covariance matrix for each error spectrum and then add the covariance matrices. A simple sum of error spectra would destroy any information about the covariance. The empirical covariance matrix generated by a single error spectrum $l$, for a specific axis $k$ (e.g., $T_\textrm{eff}$, $\log_{10} g$, or $\Z$), is
\begin{equation}
  \mathsf{C}_{ijkl}^\textrm{int} = \lambda_{ikl} \lambda_{jkl}
\end{equation}
We represent the average covariance matrix as a composite of all of the error covariance matrices of this species
\begin{equation}
  \mathsf{C}_{ijk}^\textrm{int} = \sum_{l=1}^8  d_{l} \mathsf{C}_{ijl}^\textrm{int}
\end{equation}
...
\begin{center}
  ...
  \figcaption{A typical interpolation error matrix for $T_\textrm{eff} = $ 6220K, $\log_{10} g=$4.2 dex, and $\Z = $ -0.4 dex. Some elements of the covariance matrix may be negative.}
  \label{fig:interp_matrix}
\end{center}
\end{figure}
