...
information loss.  Given these factors, we choose to develop a spectral ``emulator,'' which, given the information contained in the library of synthesized spectra, seeks to non-deterministically predict the behavior of a spectrum for stellar parameters not sampled by the library. 
...
The first step in designing an emulator is to decompose the spectral library into an appropriate basis for emulation. We use principal component analysis (PCA) to decompose the synthetic library into a set of ``eigenspectra'' $\varphi$. Following techniques of SLOAN (cite Ivecik 2014). After examining how well the spectra reconstruct the grid, we keep the first $m$ of the eigenspectra, and discard the latter eigenspectra. We keep enough eigenspectra to reconstruct any pixel flux to better than $2\%$, however most pixels are more like $XX\%$. $\varphi_k$, $k = \{1, 2, \ldots, m\}$. Before we do the PCA operation, however, we first standardize the synthetic library by subtracting the mean spectrum and ``whitening'' the spectra by dividing by the standard deviation in each pixel. First, we select only a subset of the spectra synthetic library which will be relevant to our  The mean spectrum is 
\begin{equation}
  f_{\lambda, \mu} = \frac{1}{N} \sum_{i = 1}^N f_\lambda(\{\vt_\ast \}^\textrm{grid})
\end{equation}
and the standard deviation spectrum is
\begin{equation}
  f_{\lambda, \sigma} = \sqrt{\frac{1}{N} \sum_{i=1}^N \bigl [ f_\lambda(\{\vt_\ast \}^\textrm{grid}) - f_{\lambda, \mu} \bigr]^2 } 
\end{equation}
So a spectrum from the standardized library is given by
\begin{equation}
  \widetilde{f}_\lambda(\{\vt_\ast \}^\textrm{grid}) = \frac{f_\lambda(\{\vt_\ast \}^\textrm{grid}) - f_{\lambda, \mu}}{f_{\lambda, \sigma}}
\end{equation}
Then, the eigenspectra are determined from the standardize grid using PCA analysis (see Ivecik). To speed computation, we first select a 

Using the PCA basis, we can lossily reconstruct any spectrum from the library by a linear combination of the eigenspactra
\begin{equation}
  \widetilde{f}_\lambda(\{\vt_\ast \}^\textrm{grid}) \approx \sum_{k=1}^m w_k(\{\vt_\ast \}^\textrm{grid}) \varphi_k
\end{equation}
where $w_k$ is the weight of a given eigenspectrum, $k$. This weight is a three dimensional scalar function, which depends on the stellar parameters $\vt_\ast$. By projecting the spectra from the synthetic library onto the eigenspectra, we can determine the weights at the grid points in the library, $\{\vt_\ast \}^\textrm{grid}$. For a given eigenspectrum $\varphi_k$, the weight is 
\begin{equation}
  w_k(\{\vt_\ast \}^\textrm{grid}) = \sum_\lambda \widetilde{f}_\lambda(\{\vt_\ast \}^\textrm{grid}) \varphi_k
\end{equation}
These weights are generally a smooth function of stellar parameters.
