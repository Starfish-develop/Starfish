...
\comm{The following are some suggested paragraph outlines for a concise introduction:} 

\comm{(1) start general: vast amount of information in an astronomical spectrum, and we want to 
make the most of that to learn about various physical parameters of interest.  accurate, unbiased 
stellar parameters (like temperature, gravity, metallicity, etc.) are of crucial importance in many 
astrophysics problems; e.g., exoplanets (and explain why), etc. (pick another one or two).}

\comm{(2) point out the challenge: quantitative extraction of these physical parameters is not 
easy.  go through some of the basic underlying limitations (practical, computational, limited 
models, etc.)}

\comm{(3) highlight various techniques/approaches being used (lots of references here).}

\comm{(4) now, without criticizing those approaches, just launch into a brief summary of our goals 
and a basic outline of the paper.}


\comm{I have made a lot of edits in Section 2, but have tried to restrict comments.  When there is 
something useful to say, I have added some commented-out text in the .tex file.}

\comm{Important: Note how I have changed the naming convention in the .bib file, so that a paper is 
referenced by the first author's last name and the year, like czekala14.  Your haphazard formatting 
was driving me nuts, and you can't find anything in the bib file (plus, if you read inside the .tex 
file, my approach makes things a hell of a lot easier).}
...
that interpolated atmosphere to synthesize $f_{\lambda}$ \citep[e.g., as for SME;][]{vp96}.  A more 
...
\citep[e.g., the method of SPC;][]{blj+12}.  The difficulty with this latter approach is that the 
...
model spectrum with a (low-order) Chebyshev polynomial \citep[e.g.,][]{elh+06,koleva09}, 
\begin{equation}
...
% SA: I envision this figure much like the previous one, in a 1-column vertical distribution of 
...
% SA: It may be true that the figure you've made to demonstrate this is accurate, but it certainly 
% does not seem obvious to me what is going on in it.  I like the idea of the figure, but I think 
% it would be better demonstrated on a relatively smooth (continuum) region with a small spectral 
% mismatch, rather than deep lines everywhere.  Can we do that instead?
...
For a well-designed spectrograph, this {\it global} (i.e., present throughout the spectrum) 
covariant structure should have a relatively low amplitude and small correlation length.  To 
describe that structure, we assume a stationary covariance kernel (also called a radial basis 
function) with an amplitude that depends only on the velocity distance between two pixels, 
\begin{equation}
  r_{ij} = \Delta v = \frac{c}{2} \left | \frac{\lambda_i 
...
mildly covariant structure \citep[e.g.,][]{rasmussen05}.  Here we adopt the \matern\ kernel with 
$\nu = 3/2$,
\begin{equation}
...
% SA: There was an error in this equation; the sigma in the denominator should be squared, but was 
%     not -- please check and make sure this is not manifested as a code bug.
\label{eqn:klocal}
\end{equation}
...
clipping/masking).  
% SA: The basic structure of this figure is nice.  But we should talk about making it presentation 
% quality once the other figures have been iterated as I've described (well, requested).
...
\ref{subsec:mdwarf}.  
% SA: I have to admit that I do not really understand this very well.  I think some appropriate 
...
h_k\}^{N_{\rm reg}}]$, $N_{\rm reg}$ is the number of local residual regions (see below for 
details on how this is determined), and $b$ is assumed to be a fixed parameter that scales up the 
pixel noise values to account for interpolation uncertainties (see Section \ref{subsec:synthetic}; 
reasonable values are $b \approx 1.02$--1.10 for optical spectra; see Section \ref{sec:examples} 
for examples).  
