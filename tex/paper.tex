...
Model library spectra, $f_{\lambda}(\{\vt_{\ast}\}^\textrm{grid})$, are stored as (1-dimensional) arrays of flux 
values, sampled on high resolution wavelength grids (typically $R \gtrsim 100,000$).  In the case 
of interest here, the collections of model parameters $\{\vt_{\ast}\}^\textrm{grid} = [\{T_{\rm eff}, \log g,  \Z 
...
\begin{figure*}[!t]
  ...
  \figcaption{ (\emph{top}) The mean spectrum, standard deviation spectrum, and five eigenspectra that form the basis of the \textsc{Phoenix} synthetic library used to model Gl51. (\emph{bottom}) The original synthetic spectrum from the \textsc{Phoenix} library compared with a spectrum reconstructed from a linear combination of the eigenspectra using Eqn~\ref{eqn:reconstruct} with the weights listed in the figure.
\label{fig:pca_reconstruct}}
  \end{center}
\end{figure*}

The first step in designing an emulator is to decompose the spectral library into an appropriate basis for emulation \citep{habib07, heitmann09}. We chose the principal component basis (PCA) to decompose the synthetic library into a set of ``eigenspectra,'' following the techniques of \citet{ivezic13}. Prior to the PCA decomposition, we first identify a subset of $M$ synthetic spectra which will be relevant to the star at hand. We then standardize the synthetic library by first subtracting off the mean spectrum and then ``whiten'' the spectra by dividing by the standard deviation measured in each pixel across the grid. The mean spectrum is 
\begin{equation}
  \xi_\mu = \frac{1}{N} \sum_{i = 1}^N f_\lambda(\{\vt_\ast \}^\textrm{grid}_i)
\end{equation}
and the standard deviation spectrum is
\begin{equation}
  \xi_\sigma = \sqrt{\frac{1}{N} \sum_{i=1}^N \bigl [ f_\lambda(\{\vt_\ast \}^\textrm{grid}_i) - \xi_\mu \bigr]^2 } 
\end{equation}
$\{\vt_\ast \}^\textrm{grid}$ denotes the full collection of stellar parameters sampled in the spectral library, while $\{\vt_\ast \}^\textrm{grid}_i$ denotes a single set of stellar parameters drawn from this collection. All spectra in the library are then standardized by subtracting the mean spectrum and dividing by the standard deviation spectrum
\begin{equation}
  \widetilde{f}_\lambda(\{\vt_\ast \}^\textrm{grid}) = \frac{f_\lambda(\{\vt_\ast \}^\textrm{grid}) - \xi_\mu}{\xi_\sigma}
\end{equation}
Then, the eigenspectra are computed from the standardized grid using PCA analysis (\cite{ivezic13}). We decide to truncate our basis to the first $m$ of the eigenspectra, where $m$ is decided by the minimum number of eigenspectra required to reproduce any spectrum in the grid to better than 2\% accuracy for all pixels; however the mean pixel error is generally much smaller than this ($\lesssim 0.5\%$). We denote a single $N_{\rm pix}$ eigenspectrum as $\xi_k$, with principal component index $k = \{1, 2, \ldots, m\}$. As an example, the eigenspectra basis computed for Gl51 using the \textsc{Phoenix} library is shown in Fig~\ref{fig:pca_reconstruct}, top panel.

Using the PCA basis, we can lossily reconstruct any spectrum from the library with a linear combination of the eigenspectra
\begin{equation}
  f_\lambda(\{\vt_\ast \}^\textrm{grid}_i) \approx \xi_\mu + \xi_\sigma \sum_{k=1}^m w_k(\{\vt_\ast \}^\textrm{grid}_i) \xi_k
  \label{eqn:reconstruct}
\end{equation}
where $w_k$ is the weight of a given eigenspectrum, $k$. This weight is a three dimensional scalar function which depends on the stellar parameters $\vt_\ast$. We can determine the weights at the grid points in the library by taking the dot product of the standardized synthetic spectrum with each of the eigenspectra. The weight for a given eigenspectrum $\xi_k$ is 
\begin{equation}
  w_k(\{\vt_\ast \}^\textrm{grid}_i) = \sum_\lambda \widetilde{f}_\lambda(\{\vt_\ast \}^\textrm{grid}_i) \xi_k
\end{equation}
which is generally a smooth function of stellar parameters (see Figure~\ref{fig:GP_interp}, left panel). To simplify the notation, we can write the weights in a length-$m$ column vector
\begin{equation}
  \mathbf{w}(\{\vt_\ast \}^\textrm{grid}_i) = 
  \begin{bmatrix}
    w_1(\{\vt_\ast \}^\textrm{grid}_i)\\
    w_2(\{\vt_\ast \}^\textrm{grid}_i)\\
    \vdots\\
    w_m(\{\vt_\ast \}^\textrm{grid}_i)
  \end{bmatrix}
\end{equation}
...
\end{equation}
Where $I_{N_{\rm pix}}$ is an $N_{\rm pix}$ by $N_{\rm pix}$ identity matrix. To recapitulate thus far, we have decomposed the spectra in the synthetic library into a principal component basis, allowing us to lossily reconstruct any spectrum in the synthetic library as a weighted linear combination of $m$ eigenspectra. The weight corresponding to each eigenspectrum is a moderately-smooth scalar function of the three stellar parameters. Therefore, to create a spectrum corresponding to a value of stellar parameters which is not in the spectral library, we must interpolate the weights to this new value. In practice, it might be possible to use a traditional interpolation scheme such as spline interpolation to interpolate the weights directly. However, we found that with high-signal to noise spectra (e.g., Gl51 is S/N $> 400$), the error in any given interpolated representation of the spectrum could constitute a significant portion of the uncertainty budget. This, combined with the Nyquist undersampling of the synthetic grid caused behavior where stellar parameters corresponding to a grid point in the synthetic library might be preferentially favored and bias the results, even after empirically accounting for interpolation error by doing ``drop-out'' interpolation tests. 

Now, we describe how we design the Gaussian process used for modelling the eigenspectra weights as a function of stellar parameters. In Section~\ref{sec:method}, we used the formalism of Gaussian processes to develop a sophisticated correlated noise model for the data likelihood, which include contributions from a ``global'' covariance kernel to describe the moderate-amplitude correlations present throughout the spectral residuals and several ``local'' covariance kernels to address the highly correlated but-localized spectral residuals originating from pathological errors in the synthetic spectrum. Alternatively, we now use a different Gaussian process in a separate application to serve as an interpolating function for the weights. 

We model each eigenspectrum weight independently; therefore there is a separate Gaussian process ($\mathcal{GP}$) for each eigenspectrum. For a single eigenspectrum $k$, we denote the collection of $w_k(\{\vt_\ast \}^\textrm{grid}_i)$ evaluated at all the spectra in the synthetic library as $\mathbf{w}_k^\textrm{grid}$. The Gaussian process treats $\mathbf{w}_k$ as a collection of random variables which are drawn from a single joint multi-variate Gaussian distribution \citep{rasmussen05},
\begin{equation}
  \mathbf{w}_k^\textrm{grid} \sim \mathcal{N} \left ( 0, \mathcal{K}_\textrm{SE} (\{\vt_\ast \}^\textrm{grid}, \{\vt_\ast \}^\textrm{grid})\right )
\end{equation}
...
\begin{multline}
  \mathcal{K}_\textrm{SE}(\vt_{\ast, i}, \vt_{\ast, j} | a_\textrm{SE}, l_\textrm{SE}) = a^2 \exp \left ( 
  -  \frac{(T_{\textrm{eff}_i} - T_{\textrm{eff}_j})^2}{2 l_{T_\textrm{eff}}^2 }  \right ) \times \\
  \exp \left (-\frac{(\log g_i - \log g_j)^2}{2 l_{\log g}^2} \right ) \exp \left ( -\frac{ (\Z_i - \Z_j)^2}{2 l_{\Z}^2} \right )
\end{multline}
This kernel is in contrast to the \matern\ kernel used in Section~\ref{sec:method}, which produces a more jagged Gaussian process reminiscent of the spectral residuals. Then the $M$ by $M$ covariance matrix is 
\begin{equation}
\mathsf{C}^\textrm{grid} = \mathcal{K}_\textrm{SE}( \{\vt_\ast \}^\textrm{grid}, \{\vt_\ast \}^\textrm{grid} | a_\textrm{SE}, l_\textrm{SE})
\end{equation}
which represents the evaluation of the covariance kernel for all pairings of grid point stellar parameters.
...
\begin{equation}
  p(\mathbf{w}_k^\textrm{grid} | C^\textrm{grid}) = \mathcal{N}\left (\mathbf{w}_k^\textrm{grid} \left | \; \mathbf{0}, \mathsf{C}^\textrm{grid} \right . \right )
\end{equation}
Because $\mathsf{C}^\textrm{grid}$ is defined by $\mathcal{K}_\textrm{SE}$, this likelihood function is equivalent to $p(\mathbf{w}_k^\textrm{grid} |\; a_\textrm{SE}, l_\textrm{SE})$. By multiplying this with a suitable prior over $a_\textrm{SE}$ and $l_\textrm{SE}$, we arrive at a posterior distribution for the hyperparameters of the Gaussian process. For computational efficiency, we optimize the hyperparameters by finding the peak of the posterior distribution with a downhill gradient algorithm and then consider the hyperparameters fixed.

\begin{figure*}[!t]
\begin{center}
  \includegraphics{figs/GP_left.pdf}
  \includegraphics{figs/GP_right.pdf}
  \figcaption{ (\emph{left}) the blue dots mark the weights $\mathbf{w}_5^\textrm{grid}$ of the 5th eigenspectrum $\xi_5$ for $\vt_\ast$ for various values of $T_\textrm{eff}$ with $\log g = 5.0$ and $\Z = 0.0$. The thin blue lines show 50 random draws of possible functional forms described by the Gaussian process, constrained to pass through the grid weights. (\emph{inset}) a zoomed portion showing the scatter in the possible functional forms. The black vertical line represents a slice through the scatter of the predicted weight value at $\vt_\ast = [T_\textrm{eff} = 3150\,\textrm{K}, \log g = 5.0\,\textrm{dex}, \Z = 0.0\, \textrm{dex}]$. (\emph{right}) The posterior probability of $w_k$ at this value of $\vt_\ast$ is completely described by Eqn~\ref{eqn:conditional_gp}, allowing us to analytically marginalize over all probable values of the weights, and thus marginalize over all probable spectral interpolations.
\label{fig:GP_interp}}
\end{center}
\end{figure*}

Now, with the Gaussian process specified, we can use it to predict the values of the weights at a new set of stellar parameters $\vt_\ast$ by considering them drawn from joint distribution 
\begin{equation}
  \begin{bmatrix}
  ...
  \mathbf{0},
  \begin{bmatrix}
    \mathsf{C}^\textrm{grid} & \mathcal{K}_\textrm{SE}( \{\vt_\ast \}^\textrm{grid}, \vt_\ast) \\
\mathcal{K}_\textrm{SE}(\vt_\ast, \{\vt_\ast \}^\textrm{grid}) & \mathcal{K}_\textrm{SE}(\vt_\ast,\vt_\ast)
  \end{bmatrix}
  \right )
\end{equation}
where the dependence of the covariance kernel on the optimized hyperparameters is implied. Notation means a multivariate Gaussian with $\mu$ vector = 0, length $M$. And then $\mathcal{K}_\textrm{SE}( \{\vt_\ast \}^\textrm{grid}, \vt_\ast)$ represents a length $M$ vector which is horizontally stacked to  $\mathsf{C}^\textrm{grid}$. The combination of all of these products results in a total covariance matrix that is size ($M + 1$ by $M + 1$). The probability distribution of $w_k(\vt_\ast)$ is given by the conditional Gaussian distribution \citep{rasmussen05}
\begin{equation}
...
where
\begin{equation}
 \mu_k =   \mathcal{K}_\textrm{SE}(\vt_\ast, \{\vt_\ast \}^\textrm{grid}) \mathsf{C}^{\textrm{grid}, -1} \mathbf{w}_k^\textrm{grid}
\end{equation}
\begin{multline}
  \sigma_k = \mathcal{K}_\textrm{SE}(\vt_\ast,\vt_\ast) - \\
  \mathcal{K}_\textrm{SE}(\vt_\ast, \{\vt_\ast \}^\textrm{grid}) \mathsf{C}^{\textrm{grid}, -1} \mathcal{K}_\textrm{SE}( \{\vt_\ast \}^\textrm{grid}, \vt_\ast)
\end{multline}

Although this equation is complex, the result is simpler to interpret: the probability distribution of $w_k$ is a univariate Gaussian distribution whose mean and variance are a function of $\vt_\ast$, conditional upon the (fixed) values of $\mathbf{w}_k^\textrm{grid}$ and the squared exponential hyperparameters (see Figure~\ref{fig:GP_interp}, right panel).
...
\end{equation}
Because the Gaussian process describes a probability distribution of the weights, we really have a probability distribution of possible (interpolated) models
\begin{equation}
  p(\mathsf{M} |\, \mathbf{w}) = p( \mathbf{w} |\, \vt_\ast)
\end{equation}
and the likelihood function (Eqn~\ref{eqn:likelihood}) is now also specified conditional on the weights
\begin{equation}
...
\begin{equation}
  ...
  \label{eqn:marginal}
\end{equation}
such that we are left with a modified posterior distribution of the data that incorporates the range of probable interpolation values for the model. To perform this multidimensional integral, we use a convenient lemma found in \citet[Appendix A]{gelman13}. 

If the probability distributions of $\mathbf{w}$ and $\mathsf{D} | \mathbf{w}$ are specified conditionally as in Eqns~\ref{eqn:weight_conditional} and \ref{eqn:likelihood_conditional}, respectively, then the marginal distribution (Eqn~\ref{eqn:marginal}) is
\begin{equation}
...
We can couch this in the form of Eqn~\ref{eqn:lnlikelihood} by rewriting
\begin{equation}
  \mathsf{R}^\prime = \mathsf{D} - (\widetilde{\xi}_\mu + \mathbf{X} \mathbf{\mu}_\mathbf{w})
\end{equation}
 and
\begin{equation}
  \mathsf{C}^\prime = \mathbf{X} \mathbf{\Sigma}_\mathbf{w} \mathbf{X}^T + \mathsf{C}
\end{equation}
where $\widetilde{\xi}_\mu + \mathbf{X} \mathbf{\mu}_\mathbf{w}$ can be thought of as the ``mean model spectrum'' given the model parameters, and the covariance matrix has been modified to account for the various possible interpolations.
