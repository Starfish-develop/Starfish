...
\begin{center}
  ...
  \figcaption{(\emph{top}) A small range of the ``true'' {\sc Phoenix} spectrum at $T_\textrm{eff} = $ 6000K, $\log_{10} g=$4.0 dex, and $\Z = $ -0.5 dex, shown with the drop-out interpolation spectra along each of the three parameter axes. These interpolation error spectra are determined over an interpolation distance twice that (e.g., 100K) of the actual distance used in practice. Typically, the interpolated spectra closely match the ``true'' spectrum. (\emph{bottom}) The three ``error spectra'', or difference between the interpolated spectrum and the true spectrum, relative to the true spectrum value. The shaded region shows the total change in flux between the two spectra used for interpolation, relative to the true spectrum. Note that the error spectrum is generally much smaller than the typical change in flux. For the few points where this is not true (e.g., $\log_{10} g$ near 5161.5\AA) this implies a non-monotonic change in flux between grid points. Therefore, it is best to conservatively use the error spectrum, which protects against assumptions of monotonicity.}
...
\begin{center}
  ...
  \figcaption{A typical interpolation error matrix for $T_\textrm{eff} = $ 6220K, $\log_{10} g=$4.2 dex, and $\Z = $ -0.4 dex.}
